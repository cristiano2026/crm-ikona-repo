...
import uuid
# Importa la classe datetime dal modulo datetime.
# Questo ci permette di usare "datetime" direttamente.
...
# Ho usato il decoratore per una sintassi più pulita
@app.template_filter('date')
def date_format(value, format="%d/%m/%Y"):
    """
    Format a date object to a different string format.
    Ora controlla se l'oggetto è della classe datetime, non datetime.date.
    """
...
def show_client(client_id):
    ...
    # Prepara una lista di dizionari per il template, combinando dati del servizio e dell'associazione
    ...
    for cs in client_services:
        if cs['client_id'] == client_id:
            ...
            if service:
                # Crea un nuovo dizionario che contiene sia i dati dell'associazione che i dettagli del servizio
                combined_service = {
                    'id': cs['id'], # ID dell'associazione servizio-cliente
                    'service': service, # Dettagli del servizio completo
...
def add_client_service(client_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Gestisce il caso in cui end_date_str sia vuoto
            ...
            # Aggiunge il reindirizzamento corretto dopo l'errore
            return redirect(url_for('add_client_service', client_id=client['id']))
...
def view_client_service(client_id, client_service_id):
    ...
    """
    # Cerca il cliente e l'associazione al servizio
    client = next((c for c in clients if c['id'] == client_id), None)
    client_service = next((cs for cs in client_services if cs['id'] == client_service_id), None)

    # Se non trovi uno dei due, reindirizza con un messaggio di errore
    if not client or not client_service:
        flash('Cliente o servizio associato non trovato.', 'error')
        return redirect(url_for('show_clients'))

    # Trova il servizio originale (per avere i dettagli completi)
    ...
    # Renderizza il template con tutti i dati necessari
